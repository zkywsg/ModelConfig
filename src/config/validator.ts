...
function assertConfig(condition: boolean, message: string, details?: Record<string, unknown>): void {
  if (!condition) {
    throw ModelConfigError.configInvalid(message, details);
  }
}

function validateLogicalModel(modelName: string, modelValue: unknown, envName: string): void {
...
function validateProvider(providerName: string, providerValue: unknown, envName: string): void {
...
export function validateConfig(config: unknown): asserts config is ModelConfig {
